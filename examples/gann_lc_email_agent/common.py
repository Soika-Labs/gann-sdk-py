...
def load_config() -> AppConfig:
    ...
    return AppConfig(
        ...
        gmail_history_path=_env("GMAIL_HISTORY_PATH", default=".gmail_history_id") or ".gmail_history_id",
    )
...
def fetch_all_inbox_emails(gmail_service, max_results: int = 30) -> list[EmailMessage]:
    """
    Fetch all INBOX emails regardless of read/unread status.

    Unlike fetch_unread_emails, this does NOT filter by the UNREAD label,
    so it includes emails that the automation agent has already marked as read.
    Used by the Chainlit chat interface so users can query their full inbox
    even after the auto-agent has processed and marked all mail as read.
    """
    max_attempts = 3
    result = {}
    for attempt in range(1, max_attempts + 1):
        try:
            result = gmail_service.users().messages().list(
                userId="me",
                labelIds=["INBOX"],   # INBOX only — no UNREAD filter
                maxResults=max_results,
            ).execute()
            break
        except Exception as exc:
